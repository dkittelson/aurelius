...
class TemporalTrainer:
    ...
    # --- Loads pre-processed graph ---
    def setup_data(self, dataset="elliptic"):
    ...
    # --- Splits graph into 49 snapshots ---
    def _build_snapshots(self):
        ...
        for t in unique_ts:
            # find nodes that belong to timestep t
            mask = timesteps == t
            local_ids = mask.nonzero(as_tuple=True)[0]

            # tensor remap: global index -> local index
            ...
            self.global_ids_list.append(local_ids)  # stored separately, not on snap

    # --- Builds TemporalAureliusGAT ---
    ...
    def train_temporal(self, epochs: Optional[int] = None) -> dict:
        ...
        # --- Loss: upweight illicit to fix class imbalance ---
        y = self.data.y
        labeled = y[y != -1]
        n_licit = int((labeled == 0).sum())
        n_illicit = int((labeled == 1).sum())
        class_weights = torch.tensor([1.0, n_licit / max(n_illicit, 1)], dtype=torch.float32, device=self.device)
        criterion = torch.nn.CrossEntropyLoss(weight=class_weights)

        # --- Optimizer + early stopping ---
        optimizer = torch.optim.Adam(self.model.parameters(), lr=training_cfg.get("lr", 0.001), weight_decay=training_cfg.get("weight_decay", 5e-4))
        ...
        for epoch in range(1, epochs + 1):
            ...
            # --- Train: snapshots 1-34, memory resets each epoch ---
            self.model.train()
            ...
            for i in range(train_snaps):
                ...
                # Truncated BPTT: backward every step, optimizer step every 5
                loss.backward()
            ...
            # --- Validate: snapshots 35-42 ---
            avg_loss = total_loss / max(num_steps, 1)
            val_metrics = self._evaluate_snapshots(train_snaps, val_end)
            ...
            if epoch % 10 == 0 or epoch == 1:
                ...
                print(f"Epoch {epoch:03d}/{epochs} | loss={avg_loss:.4f} | val_auprc={val_metrics['auprc']:.4f} | elapsed={elapsed:.0f}s")

            # --- Checkpoint + early stopping ---
            if val_metrics["auprc"] >= early_stop.best_score:
                torch.save({"epoch": epoch, "model_state_dict": self.model.state_dict(), "val_auprc": val_metrics["auprc"]}, checkpoint_path)
    ...
    # --- Load pre-trained GNN encoder weights ---
    def load_pretrained_encoder(self, path: str) -> None:
        if self.model is None:
            raise RuntimeError("Call setup_model() first.")
        state = torch.load(path, map_location=self.device, weights_only=False)
        self.model.encoder.load_state_dict(state)

    # --- Evaluate on val/test snapshots ---
    def _evaluate_snapshots(self, start: int, end: int) -> dict:
        self.model.eval()
        ...
        with torch.no_grad():
            for i in range(start, end):
                ...
                if labeled_mask.sum() == 0:
                    continue
                probs = torch.softmax(logits[labeled_mask], dim=1)[:, 1]
                all_probs.append(probs.cpu().numpy())
                all_labels.append(snap.y[labeled_mask].cpu().numpy())
